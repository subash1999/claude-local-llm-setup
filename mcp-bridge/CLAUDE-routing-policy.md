...
| `local_audit` | 🟢 | 2026-04-17 Leg E: 6/6 planted-bug recall P=R=1.0 on 7B + 14B. **Line drift caveat remains:** 2026-04-17 post-Fix G re-bench produced identical output to pre-fix (7B {5,10,18,23,30,42} vs GT {9,14,20,28,37,41} = 0/6 exact, ~-4 systematic drift; 14B post-fix line numbers byte-identical to pre-fix run). Fix G wired but not effective for planted-bugs fixture — either not reaching request path or model ignores the numeric prefix. **Re-locate symbol via grep before applying any autofix.** Output remains label-only prose — for actionable remediation use cloud. |
...
| `local_feature_audit` | 🟡 partial | **2026-04-17 client-side re-verification after server fixes (see `bench/results/leg-a-rebench-2026-04-17.md`):** Fix C ≤3 file cap WORKS. Fix D +5-offset EOF loop breaker WORKS (28 fabrications → 0). Fix E scrubbed the 2 prior invented BLOCKERs inside the clean block (use-me.ts:4 null-safety, VerifyCodeScreen.tsx:24 code-length) — GOOD. **BUT:** (a) 4 NEW fabricated NITs appeared, all at `VerifyCodeScreen.tsx:132` claiming 4 different state symbols are "not used in component" — every one is actually used extensively (grep-verified), Fix D dedup missed the cluster because symbols differ, Fix E symbol-exists check passed because symbols are defined. (b) 2 real BLOCKERs from original Leg A are missing from rebench (`use-google-auth.ts:23/25 createdSessionId` cascade — Opus oracle confirmed real) — possibly over-aggressive dedup. Pathology shape changed, real-BLOCKER recall regressed. **Still spot-check every finding against source. Cap ≤3 files (server-enforced). Prod-critical auth/payment → cloud.** |
| `local_semantic_search` | 🟢 | unchanged |
| `local_deep_audit` (14B) | 🟢 | 2026-04-17 Leg E: 6/6 P=R=1.0, tighter line-number accuracy than 7B (drift 1-3 vs 7B ~4). Post-Fix G re-bench line numbers byte-identical to pre-fix — Fix G not effective on this tool either. Serialize (safe=1). |
