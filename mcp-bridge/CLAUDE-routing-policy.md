...
| `local_audit` | 🟢 | **2026-04-17 round 2 (`bench/report/ROUND2-FINAL.md`):** JSON-first output with per-finding `remediation` (Item 4, `e47e341`). Filter pipeline (snap → Fix E ±5 → dedup) now active on 7B — pre-round-2 the `- BLOCKER / path:line` shape bypassed it. Planted-bugs lines: 1/6 exact pre-round-2 → **5/6 exact post-round-2** (BUG-1 drift +1 = model-capability, SQL text has no identifier-shaped symbols to snap). BUG-3 still stochastically missed — escalate to `local_deep_audit` or cloud when it matters. Optional `LOCAL_AUDIT_AUTO_ESCALATE=1` auto-fires 14B second pass when 7B returns <2 findings on >30 LOC (Item 5, `2ae4a58`). |
| `local_summarize` | 🟢 | unchanged |
| `local_find` | 🟢 | unchanged |
| `local_review` | 🟢 | Server-side Fix G + E/D active (2026-04-17 f6c8863, ba815d4, fc98bf4). Fix E window widened to ±5 (round 2 Item 2, `f423bc9`). |
| `local_diff_review` | 🟢 | Server-side Fix D dedup + AP-loop-breaker active (2026-04-17 fc98bf4). |
| `local_group_commits` | 🟢 | unchanged |
| `local_feature_audit` | 🟡 partial | **2026-04-17 round 2 (`bench/report/ROUND2-FINAL.md`):** Fix E widened to ±5 (Item 2, `f423bc9`). New pre-E grep-snap fixes 7B's cite-a-few-lines-off pattern (Item 3, `f3690b5`). On synthetic Clerk 3-file batch (`snapshot-2026-04-17-12-20-35`): 4 of 8 findings snapped to exact Opus-oracle line; post-dedup 6 findings all exact-line. **Real-client recall still pending rerun** — the 22 % rebench figure cannot be reproduced on synthetic fixtures; if it persists after round 2 it's model-capability, not bridge. Cap ≤3 files (server-enforced). Prod-critical auth/payment → cloud. |
| `local_semantic_search` | 🟢 | unchanged |
| `local_deep_audit` (14B) | 🟢 | **2026-04-17 round 2 (`bench/report/ROUND2-FINAL.md`):** JSON-first output with per-finding `remediation` (Item 4, `e47e341`) — pre-round-2 output was a backtick-wrapped line-only shape that parse-bypassed the filter pipeline. Planted-bugs still 6/6 exact ({9, 14, 20, 28, 37, 41}). **Primary route for any audit where line precision matters.** Serialize (safe=1). |
